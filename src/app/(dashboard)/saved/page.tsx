    ...
    <div className="max-w-5xl mx-auto">
            ...
            <button
              ...
              className={`
                ...
                ${statusFilter === status 
                  ...
                  : 'bg-[rgba(255,255,255,0.05)] text-[var(--color-locus-muted)] hover:text-white hover:bg-[rgba(255,255,255,0.08)]'
                }
              `}
            >
      ...
      <div className="grid grid-cols-3 gap-4 mb-8 animate-fade-in stagger-2">
        <Card className="p-4">
          ...
          <p className="text-sm text-[var(--color-locus-muted)]">Total Articles</p>
        </Card>
        <Card className="p-4">
          <p className="text-2xl font-bold text-white">{articles.filter(a => a.status === 'draft').length}</p>
          <p className="text-sm text-[var(--color-locus-muted)]">Drafts</p>
        </Card>
        <Card className="p-4">
          <p className="text-2xl font-bold text-white">{articles.filter(a => a.status === 'published').length}</p>
          <p className="text-sm text-[var(--color-locus-muted)]">Published</p>
        </Card>
      </div>
      ...
      {filteredArticles.length > 0 ? (
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  {/* Platform Icon */}
                  <div className="w-10 h-10 rounded-xl bg-[var(--color-locus-border)] flex items-center justify-center flex-shrink-0">
                    <PlatformIcon size={20} className="text-[var(--color-locus-muted)]" />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4 mb-2">
                      ...
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <div className="relative">
                          ...
                          {openMenu === article.id && (
                            <div className="absolute right-0 top-full mt-1 w-40 bg-[var(--color-locus-card)] border border-[var(--color-locus-border)] rounded-xl py-1 z-10 shadow-xl">
                              <button
                                onClick={() => handleDuplicate(article)}
                                className="w-full px-4 py-2 text-left text-sm text-[var(--color-locus-text)] hover:bg-[rgba(255,255,255,0.05)] flex items-center gap-2"
                              >
                                <Copy size={14} />
                                Duplicate
                              </button>
                              <button
                                ...
                                className="w-full px-4 py-2 text-left text-sm text-[var(--color-locus-error)] hover:bg-[rgba(239,68,68,0.1)] flex items-center gap-2"
                              >
                                <Trash2 size={14} />
                                Delete
                              </button>
                    ...
                    <p className="text-sm text-[var(--color-locus-muted)] line-clamp-2">
        ...
        <Card className="animate-fade-in text-center py-16">
          <div className="w-16 h-16 rounded-2xl bg-[var(--color-locus-border)] flex items-center justify-center mx-auto mb-4">
            <FileText className="text-[var(--color-locus-muted)]" size={28} />
          </div>
          <p className="text-[var(--color-locus-muted)] mb-2">
            {searchQuery || statusFilter !== 'all' 
              ? 'No articles match your search' 
              : 'No saved articles yet'
            }
          </p>
          <p className="text-sm text-[var(--color-locus-muted)] opacity-75 mb-6">
            {searchQuery || statusFilter !== 'all' 
