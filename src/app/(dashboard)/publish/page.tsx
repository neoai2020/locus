      ...
      <Card className="mb-8 animate-fade-in stagger-1 border-[rgba(245,158,11,0.3)] bg-[rgba(245,158,11,0.05)]">
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl bg-[rgba(245,158,11,0.15)] flex items-center justify-center flex-shrink-0">
            <AlertTriangle size={20} className="text-[var(--color-burst-warning)]" />
          </div>
          <div>
            <h3 className="font-semibold text-white mb-1">Manual Posting Required</h3>
            <p className="text-sm text-[var(--color-burst-muted)]">
              Burst generates content but does <strong className="text-white">not auto-post</strong> to any platform. 
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {platformCards.map((platform, index) => (
              <Card 
                ...
                className={`
                  ...
                  ${selectedPlatform === platform.id ? 'border-[var(--color-burst-purple)] ring-2 ring-[rgba(139,92,246,0.2)]' : ''}
                `}
                ...
                {selectedPlatform === platform.id && (
                  <div className="mt-4 pt-4 border-t border-[var(--color-burst-border)]">
                    <a 
                      ...
                      className="inline-flex items-center gap-2 text-sm text-[var(--color-burst-purple)] hover:text-[var(--color-burst-violet)] transition-colors"
                    >
          ...
          {currentArticle && (
            <Card className="animate-fade-in" style={{ animationDelay: '0.6s' }}>
              ...
              <div className="bg-[rgba(255,255,255,0.02)] border border-[var(--color-burst-border)] rounded-xl p-4 mb-4">
                <h4 className="font-medium text-white mb-2">{currentArticle.title}</h4>
                <p className="text-sm text-[var(--color-burst-muted)] line-clamp-3">
          ...
          {!currentArticle && (
            <Card className="animate-fade-in text-center py-12" style={{ animationDelay: '0.6s' }}>
              <div className="w-16 h-16 rounded-2xl bg-[var(--color-burst-border)] flex items-center justify-center mx-auto mb-4">
                <Send className="text-[var(--color-burst-muted)]" size={28} />
              </div>
              <p className="text-[var(--color-burst-muted)] mb-2">No article selected</p>
              ...
              <ul className="space-y-3">
                {tips.map((tip, index) => (
                  <li key={index} className="flex items-start gap-3 text-sm">
                    <div className="w-1.5 h-1.5 rounded-full bg-[var(--color-burst-purple)] mt-2 flex-shrink-0" />
                    <span className="text-[var(--color-burst-text)]">{tip}</span>
                  </li>
                ))}
              </ul>
            ) : (
              ...
              <div className="space-y-2">
                {platformCards.filter(p => p.url !== '#').map((platform) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-sm text-[var(--color-burst-muted)] hover:text-[var(--color-burst-purple)] transition-colors"
                  >
